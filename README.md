...
yarn start
```
- На детальной странице новости счетчик комментариев показывает количество только корневых комментариев, так как из-за структуры api общее количество комментариев возможно узнать только загрузив их все, а по условию задания (и по логике вещей) вложенные комментарии подгружаются не сразу все, а по необходимости

- Может быть такое, что при нажатии на кнопку "show more" под комментарием, новые комментарии не появятся. Это происходит, потому что приложение не рендерит удаленные комментарии. А о том, что они удалены и не содержат информации, можно узнать, только загрузив их, нажав на кнопку show more
